...
// از این پس orgBalance دوباره موجودی واقعی و مستقلِ کیف پول سازمانی همین کاربر است —
// نه یک استخر مشترک بین کل پرسنل سازمانش. مصرف این کاربر (پرداخت رزرو از کیف پول
// سازمانی) فقط همین عدد را کم می‌کند و هیچ اثری روی موجودی سایر همکارانش ندارد.
// (این تایپ قبلاً — از تسک ۷ تا بند ۲۷ — orgBalance را همیشه ۰ و «منسوخ» می‌دانست
// و موجودی واقعی را در Organization.walletBalance نگه می‌داشت؛ آن مدل کنار گذاشته شد.)
export interface Wallet {
  id: string;
  userId: string; // Reference to User.id
  normalBalance: number; // In Toman (Non-withdrawable) — کیف پول شخصی کاربر
  orgBalance: number; // In Toman (Non-withdrawable) — کیف پول سازمانی مستقل و شخصی همین کاربر
...
export interface Organization {
  ...
  // 🆕 بند ۲۷: این عدد دیگر استخر مشترک قابل‌خرج نیست؛ فقط پول «تخصیص‌نیافته» را نشان
  // می‌دهد (باقیمانده‌ی خردِ تقسیم گام ۳ فایل sql/band-27-per-employee-wallet.sql، یا
  // موجودی سازمانی که هنوز هیچ پرسنلی در بالکن ثبت‌نام نکرده). موجودی واقعی و قابل‌خرج
  // هر کارمند، مستقیماً در Wallet.orgBalance خودش نگه داشته می‌شود؛ روت
  // src/app/api/admin/corporate/organizations/route.ts برای نمایش در پنل ادمین، این فیلد
  // walletBalance را با «مجموع موجودی واقعی تمام پرسنل» بازنویسی (override) می‌کند.
  walletBalance: number;
  autoChargeEnabled: boolean; // آیا شارژ خودکار دوره‌ای برای این سازمان فعال است
  autoChargeAmount: number; // مبلغی که هر بار به‌صورت خودکار، جداگانه به هرکدام از پرسنل فعلی سازمان شارژ می‌شود
...
// ==========================================
// 2.6. Organizational Number Credits (بند ۲۷)
// ==========================================
// 🆕 بند ۲۷ (بازگشت کیف پول سازمانی به موجودی مستقل هر کارمند):
// وقتی ادمین از ابزار «شارژ گروهی از لیست شماره‌ها» برای شماره‌ای استفاده می‌کند که
// هنوز در بالکن ثبت‌نام نکرده، مبلغ آن شماره تا لحظه‌ی ثبت‌نام واقعی‌اش امن اینجا نگه
// داشته می‌شود (status="PENDING")، و همان لحظه‌ی ثبت‌نام (src/app/api/auth/register/route.ts)
// خودکار به کیف پول تازه‌ساخته‌شده‌اش اعمال و status به "APPLIED" تغییر می‌کند.
export type OrganizationalNumberCreditStatus = "PENDING" | "APPLIED";

export interface OrganizationalNumberCredit {
  id: string;
  phoneNumber: string;
  organizationName: string;
  amount: number;
  status: OrganizationalNumberCreditStatus;
  appliedToUserId: string | null; // Reference to User.id (فقط وقتی status="APPLIED")
  appliedAt: Date | string | null;
  createdByAdminId: string | null; // Reference to User.id (ادمینی که این شارژ را ثبت کرد)
  createdAt: Date | string;
}
...
export interface Transaction {
  ...
  // 🆕 بند ۲۷: چون دیگر هیچ استخر مشترک سازمانی وجود ندارد، در عمل از این پس تمام
  // تراکنش‌های سازمانی همیشه یک walletId مشخص (متعلق به همان یک کارمند) دارند؛ فیلد
  // فقط برای سازگاری با تراکنش‌های تاریخی قبل از بند ۲۷ همچنان nullable مانده.
  walletId: string | null; // Reference to Wallet.id
  // 🆕 تسک ۷: وقتی این تراکنش به کیف پول یک سازمان مربوط باشد (شارژ/کسر/برداشت بابت
  // رزرو از کیف پول سازمانی)، شناسه‌ی همان سازمان اینجا هم ثبت می‌شود — برای گزارش‌گیری
  // و فیلتر کردن تراکنش‌های یک سازمان، حتی با اینکه از بند ۲۷ به بعد walletId هم همیشه پر است.
...
export interface Booking {
  ...
  // این رزرو از کیف پول (شخصی یا سازمانی) به‌صورت پیش‌پرداخت کسر شده باشد،
...
// فعال/غیرفعال‌سازی سازمان، شارژ/کسر دستی کیف پول سازمانی (چه در قالب استخر مشترک
// قدیمی، چه از بند ۲۷ به بعد به‌صورت مستقل به ازای هر کارمند) و تغییر تنظیمات شارژ
// خودکار (src/app/api/admin/corporate/organizations/**).
